...
//         -> M5.Speaker (ES8311 on Cardputer, NS4168 on Core2)
//
...
//  mantis_alarms.h.  Missing file, unreadable card, decoder failure,
//  task busy with an earlier alarm -- every path ends in a tone rather
//  than in silence.
// ═══════════════════════════════════════════════════════════════
#include "mantis_probe_cfg.h"   // shared probe constants, no platform header
#include <stdio.h>
...
#if defined(ARDUINO)
  // M5Unified, NOT M5Cardputer.h.
  //
  // This file is shared by both probes, and including the Cardputer's
  // own entry point made the Core2 build fail outright: "M5Cardputer.h:
  // No such file or directory".  A header that claims to be shared and
  // hard-includes one platform's header is not shared -- it has merely
  // never been compiled on the other one.
  //
  // M5.Speaker is the M5Unified speaker on BOTH devices: M5Cardputer
  // derives from M5Unified, so there M5Cardputer.Speaker and M5.Speaker
  // are the same object.
  #include <M5Unified.h>
  #include <SD.h>
...
static void cp_mp3_task(void *) {
    ...
    for (;;) {
        ...
        if (file && file->isOpen()) {
            ...
            out = new AudioOutputM5Speaker(&M5.Speaker, CP_MP3_CHANNEL);
            mp3 = new AudioGeneratorMP3();
            M5.Speaker.setVolume(req.volume);
